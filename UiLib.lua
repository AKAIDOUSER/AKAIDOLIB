...
local Windown = AKHUB:CreateWindow({
	TabMinimeze = true,
	Draggable = true,
	OptionsStroke = true
})

local a = Windown:AddTab({Title = "Tab 1"})
local b = Windown:AddTab({Title = "Tab 2"})
Windown:AddTab({Title = "Tab 3"})

local c = a:AddPage({
	Animation = true
})

local ab = a:AddPlayerList({
	Animation = true,
...
c:AddDropdown({
	Values = {playerrr.Name, playerrr.userId},
	Callback = function(Value)
		print(Value)
	end
})
...
c:AddSlide({
	Title = "Speed Player",
	Min = 0,
	Max = 100,  -- Set max to 100 or any other value you prefer
	Default = 16,
	Callback = function(Value)
		print(Value)
		humanoid.WalkSpeed = Value  -- Update WalkSpeed on the Humanoid
	end,
})
...
c:AddColorpicker({
	Default = Color3.fromRGB(105, 94, 255),
	Callback = function(Value)
		print(Value)
		local player = game.Players.LocalPlayer  -- Obtenha o jogador local
		local character = player.Character or player.CharacterAdded:Wait()  -- Obtenha o personagem, esperando se necessário

		-- Função para mudar a cor do personagem para amarelo
		local function changeCharacterColorToYellow()
			-- Definindo a cor amarela
			local yellow = Value -- Cor amarela

			-- Iterar por todas as partes do personagem
			for _, part in pairs(character:GetChildren()) do
				if part:IsA("BasePart") then  -- Verifica se o objeto é uma parte física (BasePart)
					part.Color = yellow  -- Altera a cor para amarela
				end
			end
		end

		-- Chama a função para mudar a cor
		changeCharacterColorToYellow()

		-- Opcional: caso o personagem seja reequacionado ou troque de personagem, altere novamente a cor
...
local fullBrightnessEnabled = false  -- Inicializa a variável para saber se o brilho total está ativado ou não

-- Função para ajustar o brilho do mapa
local function adjustBrightness(enabled)
	if enabled then
		-- Aumenta o brilho ou ajusta a iluminação do mapa
		game.Lighting.Brightness = 7  -- Valor exemplo, você pode ajustar conforme necessário
	else
		-- Restaura o brilho para o padrão
		game.Lighting.Brightness = 3  -- Valor padrão, pode ser ajustado
	end
end

-- Função chamada quando o toggle é acionado
local function toggleFullBrightness(value)
	fullBrightnessEnabled = value
	adjustBrightness(fullBrightnessEnabled)  -- Ajusta o brilho conforme o valor do toggle
	print("Full Brightness is " .. (fullBrightnessEnabled and "enabled" or "disabled"))
end


d:AddToggle({
	Title = "Full Brightness",
	Description = "Full Brightness Map",
	Callback = toggleFullBrightness
})

game.Lighting.TimeOfDay = "03:33:33"

d:AddDropdown({
	Title = "Set Times",
	Description = "Times day",
	Values = {"Manha", "Meio Dia", "Tarde", "Noite", "Madrugada"},
	Callback = function(Value)
		if Value == "Manha" then
			game.Lighting.TimeOfDay = "07:00:00"
		elseif Value == "Meio Dia" then
			game.Lighting.TimeOfDay = "12:00:00"
		elseif Value == "Tarde" then
			game.Lighting.TimeOfDay = "17:00:00"
		elseif Value == "Noite" then
			game.Lighting.TimeOfDay = "20:00:00"
		elseif Value == "Madrugada" then
			game.Lighting.TimeOfDay = "03:33:33"
		end
	end
})
